        ...
        if(!$resultado){
            ...
            $sql_One="SELECT MAX(iddatos_usuario) AS id FROM datos_usuarios";
                ...
                while($nuevo =  $resultado_One->fetch_array()){
                        $id_usuario = $nuevo['id'];
                        $sql_seg="UPDATE datos_usuarios SET firma_dato=CONCAT(LEFT(nombre,1),LEFT(apellidoP,1),LEFT(apellidoM,1),'Coordinador',LEFT(iddatos_usuario,2)) WHERE iddatos_usuario='$id_usuario'";
                        $resultado_seg =mysqli_query($conexion,$sql_seg);
    
                        $sql_thr="UPDATE datos_usuarios SET usuario=CONCAT(LEFT(nombre,1),LEFT(apellidoP,1),LEFT(apellidoM,1),'Coordinador',LEFT(iddatos_usuario,2)) WHERE iddatos_usuario='$id_usuario'";
                        $resultado_thr =mysqli_query($conexion,$sql_thr);
    
                        $sql_for="UPDATE datos_usuarios SET password=CONCAT(LEFT(nombre,1),LEFT(apellidoP,1),LEFT(apellidoM,1),'Coordinador',LEFT(iddatos_usuario,2)) WHERE iddatos_usuario='$id_usuario'";
